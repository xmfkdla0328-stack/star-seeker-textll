// ============================================================
// story.js — 게임 콘텐츠 데이터
// 이후 스토리/노드/분기를 추가할 때는 이 파일만 수정하면 됩니다.
// engine.js 는 이 데이터를 읽어서 렌더링/판정을 처리합니다.
// ============================================================
 
// 스탯 목록 (순서가 상단 스탯 패널 표시 순서)
const STAT_KEYS = ["체력","솜씨","지력","매력","직감"];
 
// 캐릭터 생성 시 고를 수 있는 파견 계열
const ORIGINS = {
...
// {
//   tag: "화면 상단에 뜨는 라벨",
//   text: "본문" 또는 () => `동적 본문 (state 참조 가능)`,
//   onEnter: () => { 노드 진입 시 즉시 실행할 효과 (선택) },
//   choices: [
//     {
//       text: "버튼 텍스트",
//       goto: "다음 노드 id",              // 즉시 이동
//       onSelect: () => { ... },           // 선택 시 효과 (선택)
//       require: { keyword: "필요 키워드" }, // 있으면 키워드 없을 때 버튼 잠김
//       diceCheck: {                       // 있으면 주사위 판정 후 분기
//         stat: "지력",                    // 보정으로 쓸 스탯
//         dc: 4,                           // 목표 수치
//         onSuccess: () => { ... },        // 성공 시 효과 (선택)
//         success: "성공 시 이동할 노드 id",
//         fail: "실패 시 이동할 노드 id"
//       }
//     }
//   ]
